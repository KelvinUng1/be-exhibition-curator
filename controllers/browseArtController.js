...
const getBrowseArtworks = async (req, res) => {
  ...
  try {
    ...
    const aicArtworkIds = aicCollectionResponse.data.data.map(artwork => artwork.id);
    ...
    const cmaResponse = await axios.get('https://openaccess-api.clevelandart.org/api/artworks/', {
      params: {
        q: 'painting', 
        skip: startIndex,
        limit: limit,
